...
np.save('model_cnn/mean_input.npy',mean_input)
np.save('model_cnn/std_input.npy' ,std_input)
np.save('model_cnn/mean_output.npy',mean_output)
np.save('model_cnn/std_output.npy',std_output)
...
from blocks.CNN_3D import ConvUnit, UnitaryUnit, BlockCNN

layers = [
        ConvUnit(in_size = 41,  out_size = 64,   kernel = 3,  prob = 0.0),
        ConvUnit(in_size = 64,  out_size = 128,   kernel = 3,  prob = 0.0),
        ConvUnit(in_size = 128,  out_size = 256,   kernel = 3,  prob = 0.0),
        ConvUnit(in_size = 256,  out_size = 512,   kernel = 3,  prob = 0.0),
        ConvUnit(in_size = 512,  out_size = 512,   kernel = 3,  prob = 0.0),
        UnitaryUnit(in_size = 512)
        ]
...
opti = torch.optim.Adam(CNN.parameters(), lr = 8e-5, weight_decay = 8e-5)#, betas = (0.9, 0.999), eps = 1e-8, amsgrad = True )
...
print ("count parameters = \t", count_parameters(CNN))
