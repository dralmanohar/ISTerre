...
def plot_merid(r, ct, a , strm=0, czoom=1, rg=0, rm=0, shift=0, title='', levels=20, cbar=1, cmap=cmap, name=None, com=None, phi=None, field=None, name1=None, name2=None, trunc=None):
		"""m = plot_merid()"""
...
def plot_merid_field(r, ct, a , strm=0, czoom=1, rg=0, rm=0, shift=0, title='', levels=20, cbar=1, cmap=cmap, name=None, com=None, phi=None, field=None, trunc=None):
		"""m = plot_merid()"""
		r = r.reshape(r.size,1)
		ct = ct.reshape(1, ct.size)
		st = sqrt(1-ct*ct)
		...
		fig.tight_layout()	
		# ~ plt.savefig("{0:}/nlin_{1:}_{2:}_phi_{3:}_t_9000.pdf".format(MYDIR, name, com, int(np.floor(phi))), dpi = 600)
		plt.savefig("{0:}/{1:}_{2:}_phi_{3:}_t_9000.pdf".format(MYDIR, name, com, int(np.floor(phi))), dpi = 600)
		plt.savefig("{0:}/{1:}_{2:}_phi_{3:}_t_9000.png".format(MYDIR,name, com, int(np.floor(phi))), dpi = 600)
		return m
...
# ~ field = {'Fuwr':Uwr, 'Fuwt': Uwt, 'Fuwp':Uwp, 'Fjbr':Ur, 'Fjbt': Ut, 'Fjbp':Up, 'Fubr':Wr, 'Fubt':Wt, 'Fubp':Wp}
# ~ field = {'Fuur':Ur, 'Fuut': Ut, 'Fuup':Up, 'Fbbr':Wr, 'Fbbt':Wt, 'Fbbp':Wp}
# ~ field = {'Fubr':Ur, 'Fubt': Ut, 'Fubp':Up, 'Fbur':Wr, 'Fbut':Wt, 'Fbup':Wp}
...
# ~ for ang in phi_cut:
	...
	# ~ for keys, value in field.items():
		...
		# ~ ur = value[:,:,ang]/factor
		# ~ plot_merid(r, cos(theta), ur, cmap=cmap, czoom=5, levels=20, title = 'com' , name = name, com = com, phi=degree, field = com, name1=name1, name2=name2, trunc='trunc_F_zoom')
# ~ plt.show()
