...
count = 0
info,r =get_field_info('/nfs_scratch/sharmam/Nonlinear_term/data/Thomas_data_ite_save_10/data_E1e-4_Ra1.5e8_Pm3_hdiff_qo0.5_dqo0_Y20_res1_itr_10/fieldU_0000.E1e-4_Ra1.5e8_Pm3_hdiff_qo0.5_dqo0_Y20_res1_movie')
...
def load_fieds(A, lcut, mcut):
    ...
    Utrr = np.zeros((Nr, Nlat, Nphi));	Utrt = np.zeros((Nr, Nlat, Nphi));	Utrp = np.zeros((Nr, Nlat, Nphi))
    ...
    try:
        ...
        for ir in range(Ulm.irs, Ulm.ire+1):
            ...
            utrr_lm = truncate_l_m(ur_lm, lmax, mmax, lcut, mcut)
            utrs_lm = truncate_l_m(us_lm, lmax, mmax, lcut, mcut)
            utrt_lm = truncate_l_m(ut_lm, lmax, mmax, lcut, mcut)

            ur, ut, up = sh.synth(ur_lm, us_lm, ut_lm)
            utrr, utrt, utrp = sh.synth(utrr_lm, utrs_lm, utrt_lm)
    ...
    return irs, ire, r, theta, phi, Ur, Ut, Up, Utrr, Utrt, Utrp
...
def snapshot_nonline_computation(filenameu, filenameb, filenamet, lcut=None, mcut=None, path = None, save_path = None, time_stamp=None):
    ...
    irs, ire, r, theta, phi, Ur, Ut, Up, Utrr, Utrt, Utrp = load_fieds(path + filenameU, lcut, mcut)
    _,_,_,_,_, Br, Bt, Bp, Btrr, Btrt, Btrp 	  = load_fieds(path + filenameB, lcut, mcut)

    Br, Bt, Bp = Br[irs:ire+1], Bt[irs:ire+1], Bp[irs:ire+1]
    Btrr, Btrt, Btrp = Btrr[irs:ire+1], Btrt[irs:ire+1], Btrp[irs:ire+1]

    r,theta,phi,T,Ttr, time1 = load_fieds_scalar(path + filenameT,lcut, mcut)

    TDV1, TDV2, TDV3, TDV4, TDV5, TDV6, TDV7, TDV8, TDV9, DV1, DV2, DV3, DV4, DV5, DV6, DV7, DV8, DV9 = grad_vector(Utrr, Utrt, Utrp, r, theta, phi)
    TDB1, TDB2, TDB3, TDB4, TDB5, TDB6, TDB7, TDB8, TDB9, DB1, DB2, DB3, DB4, DB5, DB6, DB7, DB8, DB9 = grad_vector(Btrr, Btrt, Btrp, r, theta, phi)
    TDT1, TDT2, TDT3, DTt, DTp           = grad_scalar(Ttr, r, theta, phi)
    ...
    UTtr = non_lin_temp(Utrr, Utrt, Utrp, Ttr, r, theta, phi)

    uutrr, uutrt, uutrp = non_lin_same_vector(Utrr, Utrt, Utrp, r, theta, phi)
    bbtrr, bbtrt, bbtrp = non_lin_same_vector(Btrr, Btrt, Btrp, r, theta, phi)
    
    ubtrr, ubtrt, ubtrp = non_lin_diff_vector(Utrr, Utrt, Utrp, Btrr, Btrt, Btrp, r, theta, phi, 'uGb')
    butrr, butrt, butrp = non_lin_diff_vector(Utrr, Utrt, Utrp, Btrr, Btrt, Btrp, r, theta, phi, 'bGu')
    ...
    return nonlin_full, nonlin_trunc, dim, tempra, time1
